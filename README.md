...
**Science AI Chatbot**은 초등학생들의 학습을 돕는 차세대 AI 교육 플랫폼입니다. 현재는 AI가 **학습 자료의 실제 내용을 분석하고 학생 질문과 맥락에 맞는 구체적 내용을 제공하는 지능형 학습 파트너**로 발전했습니다.

### ✨ 핵심 특징
- 🧠 **적응형 AI 튜터**: 학생 응답 유형을 실시간 분석하여 6가지 맞춤형 교수 전략 자동 적용
- 🔍 **지능형 자료 처리**: PDF, 이미지 OCR, 웹 스크래핑을 통해 실제 자료 내용을 분석하고 AI가 맥락에 맞게 활용
- 📚 **교육 관리 플랫폼**: 수업 생성, 학생 관리, 수업 자료 탑재(웹페이지, PDF), 성과 분석까지 완전한 교육 워크플로우 지원
- 👩‍🏫 **교사 중심 설계**: 직관적인 대시보드로 수업 관리, AI 프롬프트 편집, 학생 분석 가능, 수업별 챗봇 및 자료 관리, 과목별 탬플릿 프롬프트 제공
...
- **학생용 메인(교사가 제시한 QR코드로 접속가능)**: https://aitutor.sangcode.dev
- **교사 대시보드**: https://aitutor.sangcode.dev/teacher.html
- **소개 페이지**: https://aitutor.sangcode.dev/introduction.html
...
- **지능형 자료 처리**: pdf-parse (PDF 텍스트 추출), tesseract.js (OCR), cheerio + axios (웹 스크래핑)

## 🚀 주요 기능

### 👨‍🏫 교사용 대시보드
- **Google OAuth 인증**: 안전한 교사 인증 시스템
- **수업 관리**: 수업 생성, 편집, QR 코드 자동 생성 및 자료 업로드, 수업 자료(웹링크, PDF)탑재
- **학생 관리**: 제출 현황 실시간 모니터링
- **채팅 관리**: 개인 대화 저장, 제출 현황 추적, AI 피드백 자동 생성
- **AI 튜터 관리**: 커스텀 프롬프트 편집, 6가지 응답 유형별 맞춤 설정, 과목별 메인 추천 프롬프트 제공
- **성과 분석**: 학생별 대화 포트폴리오, 학습 패턴 분석, 수업별 전체 학습자의 대화 기록 CSV, 수업에 응답한 학습자에 대한 전체 분석 내용 CSV 다운로드, 학습 참여도 CSV 제공
...
- **지능형 자료 활용**: AI가 실제 PDF, 이미지, 웹페이지 내용을 분석하여 학생 질문과 관련된 구체적 내용 안내
...
- **지능형 자료 처리**: PDF/이미지/웹 콘텐츠 실제 내용 추출 및 AI 기반 관련성 분석
...
│   │   ├── contentExtractor.js    # 🆕 파일 내용 추출 (PDF, OCR, 웹스크래핑)
│   │   └── semanticSearch.js      # 🆕 의미적 자료 검색 및 관련성 분석
│   └── prompts/                # 과목별 프롬프트 설정
...
## 🏆 최신 업데이트 (2025년 9월 16일)

### 🤖 지능형 자료 처리 시스템 구축
- **실제 내용 추출**: PDF 텍스트, 이미지 OCR, 웹페이지 스크래핑으로 실제 내용 추출
- **AI 기반 관련성 분석**: 학생 질문과 자료 내용 간 의미적 유사도 계산
- **맥락적 자료 추천**: 학습 상황과 질문 패턴에 맞는 맞춤형 자료 추천
- **강화된 프롬프트**: 관련 자료의 구체적 내용을 바탕으로 정확한 AI 안내

### 🚀 기술적 업그레이드
- **ContentExtractor 모듈**: PDF 및 OCR 처리, 웹 스크래핑 시스템
- **SemanticSearch 모듈**: AI 기반 의미적 검색 및 관련성 분석
- **통합 아키텍처**: 기존 시스템과 완만한 통합, 폴백 메커니즘 포함
...
**현재 (실제 내용 기반 안내)**:
“분수를 나누는 것은 곱셈의 역연산이에요. 교사가 준비한 '분수 나눗셈 시각화 자료'에서 '피자 1개를 1/2로 나누면 2조각이 되는' 부분을 보면 더 이해하기 쉬울 거예요.”
...
### v2.1.0 - 2025.09.16 🚀
**지능형 자료 처리 시스템 전면 업그레이드**

#### Added
- 🔍 **ContentExtractor**: PDF 텍스트 추출, 이미지 OCR, 웹 스크래핑 시스템
- 🧠 **SemanticSearch**: AI 기반 의미적 유사도 및 관련성 분석
- 📝 실제 자료 내용 기반 동적 프롬프트 생성
- 📚 맥락적 자료 추천 알고리즘

#### Enhanced
- ⚡ 기존 `getTutorResponse` 함수에 지능형 처리 로직 통합
- 💬 `PromptBuilder`에 추출된 자료 내용 포함 기능
- 🛡️ 폴백 메커니즘으로 안정성 향상

#### Dependencies
- ➕ pdf-parse: PDF 파일 텍스트 추출
- ➕ tesseract.js: 한글/영문 OCR 지원
- ➕ cheerio + axios: 웹페이지 스크래핑

### v2.0.1 - 2025.08.26
**레거시 정리 및 구조 정리**
- 🔥 레거시 파일 제거: admin.html, examples/
- 🚀 Functions 정리: 불필요한 수동 등록 함수 제거
- 📁 다큐먼트 아카이브로 이동

### v2.0.0 - 2025.08.19
**전면적 교사 대시보드 리뉴얼**
- 🎆 수업 정보 편집 시스템
- 📎 자료 관리: 파일 업로드 및 링크 관리
- 📱 모바일 친화적 반응형 디자인
- 🤖 Gemini 2.0 Flash 등 최신 AI 모델 지원
- 📊 실시간 제출 현황 및 AI 피드백
